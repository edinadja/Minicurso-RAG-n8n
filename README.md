...
> Altere o usuário/senha do n8n no `docker-compose.yml` antes de distribuir o
> material, se desejar.
...
## Roteiro sugerido (1h40)

| Tempo   | Atividade                                                                 |
|---------|----------------------------------------------------------------------------|
| 0–15min | Conceitos: o que é RAG, LLMs, embeddings, chunking, bancos vetoriais       |
| 15–25min| Tour pela arquitetura e pelos containers (n8n, Qdrant, Python API)         |
| 25–55min| Construção do fluxo de **ingestão** no n8n (ler documento → Python API)   |
| 55–85min| Construção do fluxo de **pergunta e resposta** (busca + prompt + LLM)     |
| 85–95min| Testes e discussão: perguntas fora do documento, alucinações, limitações  |
| 95–100min| Encerramento e próximos passos                                           |
